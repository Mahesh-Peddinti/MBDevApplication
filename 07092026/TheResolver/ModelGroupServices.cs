...
namespace TheResolver
{
    ...
    public class ModelGroupServices:IExternalCommand
    {
        #region
        // ------------------------------------------------------------
        // UNIT CONVERSION
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            try
            {
                ...
                // ----------------------------------------------------
                // FIND GROUPED CLASHES
                // ----------------------------------------------------

                List<ClashGroupInfoDTOs> groupedClashes = geometricUtilities.FindClashGroup(clashes, settings);
                if (groupedClashes.Count == 0)
                {
                    TaskDialog.Show(
                        "Cable Tray Clash Resolver",
                        $"Cable Trays groups found : {groupedClashes.Count}\n" +
                        $"Cable Trays found : {groupedClashes.Select(g => g.Tray).Distinct().Count()}\n" +
                        "No solid clashes detected."
                    );
                    return Result.Succeeded;
                }
                ...
                {
                    tx.Start();
                    foreach (ClashGroupInfoDTOs clash in groupedClashes)
                    {
                        // ------------------------------------------------
                        // V0.1:
                        ...
                        if (processedTrays.Contains(clash.Tray.Id.Value))
                        {
                            skipped++;
                            continue;
                        }
                        ...
                        if (!geometricUtilities.TryBuildBypassRoutingClashGroupPoints(clash, 
                                                                                      settings, 
                                                                                      out routePoints))
                        {
                            skipped++;
                ...
                TaskDialog.Show(
                    ...
                    $"Cable Trays found : {trays.Count}\n" +
                    $"Conduits found    : {conduits.Count}\n" +
                    $"Clashes found     : {groupedClashes.Count}\n" +
