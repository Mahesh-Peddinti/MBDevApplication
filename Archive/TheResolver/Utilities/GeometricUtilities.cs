...
namespace TheResolver.Utilities
{
    public class GeometricUtilities
    {
                                                   ...
                                                   RouteSettingDTOs settings)
        {
            ...
            Dictionary<long, BoundingBoxXYZ> trayBoxes = trays.ToDictionary(
                                                            x => x.Id.Value,
                                                            x => x.get_BoundingBox(null));

            //Dictionary<string, BoundingBoxXYZ> clashElementBoxes =
            //                                            multiCategoryElements.ToDictionary(
            //                                                x => x.UniqueId,
            //                                                x => x.get_BoundingBox(null));

            Dictionary<long, BoundingBoxXYZ> clashElementBoxes =
            ...
            foreach (Element e in multiCategoryElements)
            {
                try
                {
                    ...
                    if (!clashElementBoxes.ContainsKey(e.Id.Value))
                    {
            ...
            {
                BoundingBoxXYZ trayBox = trayBoxes[tray.Id.Value];
                ...
                {
                    //Skip the process if the trayId is equals to any of multiCategoryElements elem
                    if(tray.Id.Value == elem.Id.Value)
                        continue;

                    BoundingBoxXYZ clashElementBox = clashElementBoxes[elem.Id.Value];

                    if (clashElementBox == null)
                        continue;
