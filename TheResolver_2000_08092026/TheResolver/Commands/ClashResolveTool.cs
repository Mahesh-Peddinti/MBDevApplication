...
namespace TheResolver.Commands
{
    public class ClashResolveTool : IExternalEventHandler
    {
        ...
        private void ExecuteBatchResolution(UIApplication app)
        {
            ...
            var selectedItems = _viewModel.Clashes.Where(c => c.IsSelected && !c.IsResolved).ToList();
            if (selectedItems.Count == 0) return;

            var routeSettings = GetCurrentRouteSettings();
            GeometricUtilities geomUtils = new GeometricUtilities { SpatialIndex = _spatialIndex };

            // Group rows by Host Tray ElementId (handling int64/long across Revit versions)
            var trayGroups = selectedItems
                .Where(c => c.ClashInfo?.Tray != null && c.ClashInfo.Tray.IsValidObject)
                .GroupBy(c => GetElementIdValue(c.ClashInfo.Tray.Id))
                .ToList();

            var processedTrayIds = new HashSet<long>();

            foreach (var group in trayGroups)
            {
                long currentTrayId = group.Key;
                if (processedTrayIds.Contains(currentTrayId)) continue;

                var primaryClash = group.First();
                CableTray hostTray = primaryClash.ClashInfo.Tray;

                // 1. Guard against invalidated/deleted elements from prior iterations
                if (!hostTray.IsValidObject)
                {
                    foreach (var item in group)
                    {
                        item.IsFeasible = false;
                        item.ResolutionLogMessage = "Host tray was modified or deleted by a prior operation.";
                    }
                    continue;
                }

                // 2. In-flight Live Check: Has another rerouted element already cleared this run?
                bool anyActiveClashes = false;
                foreach (var item in group)
                {
                    if (IsClashStillActive(doc, hostTray, item.ClashInfo?.ClashElement))
                    {
                        anyActiveClashes = true;
                        break;
                    }
                    else
                    {
                        item.IsResolved = true;
                        item.IsFeasible = true;
                        item.ResolutionLogMessage = "Cleared: Target obstacle was already rerouted away.";
                    }
                }

                if (!anyActiveClashes)
                {
                    processedTrayIds.Add(currentTrayId);
                    continue; // All obstacles for this tray are already physically cleared; keep straight!
                }

                // 3. Mutual Clash Ownership: If clashing with another selected cable tray, let the lower ID detour first
                var mutualConflict = group.FirstOrDefault(c =>
                    c.ClashInfo?.ClashElement is CableTray otherTray &&
                    trayGroups.Any(g => g.Key == GetElementIdValue(otherTray.Id)));

                if (mutualConflict != null)
                {
                    CableTray otherTray = (CableTray)mutualConflict.ClashInfo.ClashElement;
                    long otherTrayId = GetElementIdValue(otherTray.Id);

                    if (currentTrayId > otherTrayId && !processedTrayIds.Contains(otherTrayId))
                    {
                        // Defer this tray; the lower ID tray will detour around this one
                        mutualConflict.ResolutionLogMessage = "Deferred: Lower ID tray has detour priority.";
                        continue;
                    }
                }

                // 4. Solve Route considering the entire corridor
                List<XYZ> routingPoints;
                bool solved = TryResolveWithKinematicAStar(doc, hostTray, primaryClash.ClashInfo.ClashElement, routeSettings, out routingPoints);

                if (!solved || routingPoints.Count < 2)
                {
                    solved = geomUtils.TryBuildBypassRoutingPoints(primaryClash.ClashInfo, routeSettings, out routingPoints);
                }

                // 5. Generate Revit Elements and Cascade Status
                if (solved && routingPoints.Count >= 4)
                {
                    bool created = CreateBypassElements(doc, hostTray, routingPoints, routeSettings);
                    if (created)
                    {
                        processedTrayIds.Add(currentTrayId);

                        // Mark ALL clash rows sharing this Tray ID as Resolved
                        var allClashesOnThisTray = _viewModel.Clashes
                            .Where(c => c.Element1Id == currentTrayId.ToString() ||
                                       (c.ClashInfo?.Tray != null && GetElementIdValue(c.ClashInfo.Tray.Id) == currentTrayId))
                            .ToList();

                        foreach (var item in allClashesOnThisTray)
                        {
                            item.IsResolved = true;
                            item.IsFeasible = true;
                            item.ResolutionLogMessage = $"Resolved in corridor run ({routingPoints.Count} nodes).";
                        }

                        // If this resolved a mutual clash, immediately resolve the other tray's corresponding row
                        if (mutualConflict != null)
                        {
                            long otherTrayId = GetElementIdValue(mutualConflict.ClashInfo.ClashElement.Id);
                            var partnerRow = _viewModel.Clashes.FirstOrDefault(c =>
                                c.ClashInfo?.Tray != null &&
                                GetElementIdValue(c.ClashInfo.Tray.Id) == otherTrayId &&
                                c.ClashInfo.ClashElement != null &&
                                GetElementIdValue(c.ClashInfo.ClashElement.Id) == currentTrayId);

                            if (partnerRow != null)
                            {
                                partnerRow.IsResolved = true;
                                partnerRow.IsFeasible = true;
                                partnerRow.ResolutionLogMessage = "Cleared by primary tray bypass.";
                            }
                        }
                    }
                    else
                    {
                        foreach (var item in group)
                        {
                            item.ResolutionLogMessage = "Revit API rejected fitting placement.";
                        }
                    }
                }
                else
                {
                    ...
                    {
                        item.IsFeasible = false;
                        item.ResolutionLogMessage = "No collision-free path found for this run.";
                    }
                }
            }
    ...
    return id.Value;
        ...
        private bool IsClashStillActive(Document doc, CableTray tray, Element obstacle)
        {
            if (tray == null || obstacle == null || !tray.IsValidObject || !obstacle.IsValidObject)
                return false;

            // Check live physical lines if the obstacle is an active CableTray
            if (obstacle is CableTray obstacleTray)
            {
                if (!(tray.Location is LocationCurve lc1) || !(lc1.Curve is Line line1)) return false;
                if (!(obstacleTray.Location is LocationCurve lc2) || !(lc2.Curve is Line line2)) return false;

                double trayHeight = tray.get_Parameter(BuiltInParameter.RBS_CABLETRAY_HEIGHT_PARAM)?.AsDouble() ?? (100.0 / 304.8);
                double obsHeight = obstacleTray.get_Parameter(BuiltInParameter.RBS_CABLETRAY_HEIGHT_PARAM)?.AsDouble() ?? (100.0 / 304.8);
                double requiredClearance = 50.0 / 304.8;

                // If vertical separation already satisfies clearance, clash is no longer active
                double zDistance = Math.Abs(line1.GetEndPoint(0).Z - line2.GetEndPoint(0).Z);
                if (zDistance >= (trayHeight + obsHeight) * 0.5 + requiredClearance)
                {
                    return false;
                }

                // Check if projected 2D lines are disjoint
                Line flatLine1 = Line.CreateBound(
                    new XYZ(line1.GetEndPoint(0).X, line1.GetEndPoint(0).Y, 0),
                    new XYZ(line1.GetEndPoint(1).X, line1.GetEndPoint(1).Y, 0));
                Line flatLine2 = Line.CreateBound(
                    new XYZ(line2.GetEndPoint(0).X, line2.GetEndPoint(0).Y, 0),
                    new XYZ(line2.GetEndPoint(1).X, line2.GetEndPoint(1).Y, 0));

                IntersectionResultArray results;
                var intersectResult = flatLine1.Intersect(flatLine2, CurveIntersectResultOption.Simple);
